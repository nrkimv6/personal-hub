<script>
	...
	async function loadData() {
		try {
			...
			// 통계
			const statsRes = await fetch(`/api/v1/mobile/targets/${targetId}/stats`);
			if (statsRes.ok) {
				stats = await statsRes.json();
			}

			// 최근 아이템
			const itemsRes = await fetch(`/api/v1/mobile/targets/${targetId}/items?limit=20`);
			if (itemsRes.ok) {
	...
	async function executeTarget() {
		...
		try {
			const response = await fetch(`/api/v1/mobile/targets/${targetId}/execute`, {
				method: 'POST'
			});
...
<div class="container mx-auto p-4">
	...
	{:else if target}
		<div class="mb-6">
			<div class="flex justify-between items-start">
				...
				<div class="flex gap-2">
					...
					<button class="btn btn-ghost" onclick={() => goto('/mobile/targets')}>
						목록
					</button>
				</div>
			</div>
		...
		{#if stats}
			<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
				...
				<div class="stat bg-base-100 shadow">
					...
					<div class="stat-value text-success">{stats.new_items_count}</div>
				</div>
				<div class="stat bg-base-100 shadow">
					<div class="stat-title">변경 아이템</div>
					<div class="stat-value text-warning">{stats.changed_items_count}</div>
				</div>
				<div class="stat bg-base-100 shadow">
					<div class="stat-title">최근 실행</div>
					<div class="stat-value text-sm">
						{stats.latest_run_at ? new Date(stats.latest_run_at).toLocaleString() : 'N/A'}
					</div>
				</div>
			</div>
		{/if}
		...
		{#if items.length === 0}
			<div class="text-center py-8 text-gray-500">아직 수집된 아이템이 없습니다.</div>
		{:else}
					...
					<div class="card bg-base-100 shadow">
						<div class="card-body">
							<h3 class="card-title text-base">
								{item.title}
								{#if item.is_changed}
									<span class="badge badge-warning">변경됨</span>
								{/if}
							</h3>
							{#if item.item_url}
								<a href={item.item_url} target="_blank" class="text-sm text-blue-600 hover:underline break-all">
									{item.item_url}
								</a>
							{/if}
							<div class="text-sm text-gray-500">
								최초: {new Date(item.first_seen_at).toLocaleString()} |
