<script lang="ts">
	...
	let categoryMap = $derived(new Map(categories.map((c) => [c.id, c.full_path])));
	...
	function flattenCategories(cats: any[]): Category[] {
		...
		for (const cat of cats) {
			result.push({ id: cat.id, name: cat.name, full_path: cat.full_path });
	...
	async function loadFiles(reset = false) {
		...
		try {
			...
			// 신뢰도 필터 (서버사이드 지원 안될 수 있으니 클라이언트 필터도 적용)
