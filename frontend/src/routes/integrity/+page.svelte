<script lang="ts">
	...
	async function fixAll(dryRun: boolean) {
		...
		try {
			...
			if (dryRun) {
				const affectedTotal = result.results.reduce((sum, r) => sum + r.affected_count, 0);
				lastFixResult = {
					success: true,
...
<svelte:head>
	<title>데이터 정합성 검사</title>
</svelte:head>

<div class="container mx-auto p-4 max-w-6xl">
	<div class="flex justify-between items-center mb-6">
		<h1 class="text-2xl font-bold">데이터 정합성 검사</h1>
		<button
			onclick={runCheck}
			disabled={loading}
	...
	<!-- 알림 메시지 -->
	{#if lastFixResult}
		<div class="mb-4 p-3 rounded {lastFixResult.success ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}">
	...
	{#if checkResult}
		<div class="grid grid-cols-4 gap-4 mb-6">
			<div class="bg-white p-4 rounded shadow">
				...
				<div class="text-2xl font-bold">{checkResult.total_issues}</div>
			</div>
			<div class="bg-red-50 p-4 rounded shadow">
				<div class="text-sm text-red-600">Critical</div>
				<div class="text-2xl font-bold text-red-700">{checkResult.by_severity.critical}</div>
			</div>
			<div class="bg-yellow-50 p-4 rounded shadow">
				<div class="text-sm text-yellow-600">Warning</div>
				<div class="text-2xl font-bold text-yellow-700">{checkResult.by_severity.warning}</div>
			</div>
			<div class="bg-blue-50 p-4 rounded shadow">
				<div class="text-sm text-blue-600">Info</div>
				<div class="text-2xl font-bold text-blue-700">{checkResult.by_severity.info}</div>
			</div>
		</div>
			...
			<div class="bg-white rounded shadow overflow-hidden mb-6">
				<table class="w-full">
					<thead class="bg-gray-50">
						<tr>
							<th class="px-4 py-3 text-left text-sm font-medium text-gray-600">심각도</th>
							<th class="px-4 py-3 text-left text-sm font-medium text-gray-600">테이블</th>
							...
							<th class="px-4 py-3 text-right text-sm font-medium text-gray-600">수량</th>
					...
					<tbody class="divide-y divide-gray-200">
						{#each checkResult.issues as issue}
							<tr class="hover:bg-gray-50">
								...
								<td class="px-4 py-3 font-mono text-sm">{issue.table}</td>
								<td class="px-4 py-3 font-mono text-sm">{issue.issue_type}</td>
								<td class="px-4 py-3 text-sm text-gray-600">{issue.description}</td>
								<td class="px-4 py-3 text-right font-mono">{issue.count.toLocaleString()}</td>
								<td class="px-4 py-3 text-center">
									{#if issue.auto_fixable}
										<div class="flex justify-center gap-1">
											<button
												...
												class="px-2 py-1 text-xs bg-gray-100 text-gray-600 rounded hover:bg-gray-200 disabled:opacity-50"
												title="미리보기"
											>👁️</button>
											<button
									...
									{:else}
										<span class="text-xs text-gray-400">수동 처리</span>
									{/if}
								</td>
							</tr>
							<!-- 샘플 ID 표시 (있는 경우) -->
		...
		{:else}
			<div class="bg-green-50 p-6 rounded text-center text-green-700 mb-6">
				✅ 모든 정합성 검사를 통과했습니다!
			</div>
		{/if}
	{/if}
	...
	{#if dbStats}
		<div class="bg-white rounded shadow p-4">
			<h2 class="text-lg font-semibold mb-4">DB 통계</h2>
			<div class="mb-4">
				<span class="text-gray-500">DB 크기:</span>
				<span class="font-mono font-semibold">{dbStats.db_size_mb} MB</span>
			</div>
			<div class="grid grid-cols-4 gap-3">
				{#each Object.entries(dbStats.tables) as [table, count]}
					<div class="flex justify-between items-center py-1 px-2 bg-gray-50 rounded text-sm">
						<span class="font-mono text-gray-600">{table}</span>
						<span class="font-mono font-semibold">{formatCount(count)}</span>
					</div>
				{/each}
			</div>
		</div>
	...
	{#if loading && !checkResult}
		<div class="flex justify-center items-center py-12">
			...
			<span class="ml-3 text-gray-600">검사 중...</span>
		</div>
	{/if}
</div>
