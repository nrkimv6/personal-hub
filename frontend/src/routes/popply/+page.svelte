<script lang="ts">
	...
	function selectTab(tab: ActiveTab) {
		activeTab = tab;
		if (tab === 'history' && historyEvents.length === 0) void loadHistory();
	}
...
<main class="mx-auto max-w-6xl space-y-5 p-6">
	<header class="flex flex-wrap items-center justify-between gap-3">
		<div>
			...
			<p class="mt-1 text-sm text-slate-500">예약 일정과 실행내역</p>
		</div>
		<button class="rounded border px-3 py-2 text-sm" on:click={refreshAll} disabled={loading || historyLoading}>
			새로고침
		</button>
	</header>
	...
	<div class="flex gap-2 border-b">
		<button
			class="border-b-2 px-3 py-2 text-sm {activeTab === 'schedules' ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-500'}"
			on:click={() => selectTab('schedules')}
		>
			일정
		</button>
		<button
			class="border-b-2 px-3 py-2 text-sm {activeTab === 'history' ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-500'}"
	...
	{:else}
		<section class="grid gap-3 md:grid-cols-[160px_180px_180px_160px_120px]">
			<select class="rounded border px-3 py-2" bind:value={historyFilters.status}>
				<option value="">전체</option>
				<option value="available">예약가능</option>
				<option value="no_slots">매진</option>
				<option value="error">오류</option>
			</select>
			<input class="rounded border px-3 py-2" type="date" bind:value={historyFilters.date_from} />
			<input class="rounded border px-3 py-2" type="date" bind:value={historyFilters.date_to} />
			<button class="rounded bg-slate-900 px-3 py-2 text-sm text-white" on:click={searchHistory}>검색</button>
			<button class="rounded border px-3 py-2 text-sm" on:click={clearHistoryFilters}>초기화</button>
		</section>

		{#if historyError}
			<p class="rounded border border-red-200 bg-red-50 p-3 text-sm text-red-700">{historyError}</p>
		{:else if historyLoading}
			<div class="rounded border p-8 text-center text-sm text-slate-500">불러오는 중</div>
		{:else if historyEvents.length === 0}
			<div class="rounded border p-8 text-center text-sm text-slate-500">실행내역이 없습니다.</div>
		{:else}
			<section class="overflow-hidden rounded border">
				<div class="border-b bg-slate-50 px-3 py-2 text-sm text-slate-600">
						...
						<tbody>
							{#each historyEvents as event (event.id)}
								...
								<tr class="border-t align-top">
									<td class="whitespace-nowrap p-3 text-slate-600">{formatDateTime(event.timestamp)}</td>
									<td class="p-3">
										<div class="font-medium text-slate-900">{event.biz_item_name || '-'}</div>
										<div class="text-xs text-slate-500">{event.business_name || '-'}</div>
										<div class="text-xs text-slate-400">#{event.schedule_id}</div>
									</td>
									...
									<td class="p-3">
										...
										<div class="mt-1 text-xs text-slate-500">{event.available_count}개</div>
									</td>
									<td class="max-w-[300px] p-3 text-xs text-slate-600">
										{#if slots.length === 0}
											-
										{:else}
											<div class="space-y-1">
												...
												{/each}
												{#if slots.length > 3}
													<div class="text-slate-400">외 {slots.length - 3}개</div>
												{/if}
											</div>
										{/if}
									</td>
									<td class="whitespace-nowrap p-3 text-slate-600">
										...
										<div class="text-xs text-slate-400">{event.fetch_method || '-'}</div>
									</td>
									<td class="max-w-[160px] truncate p-3 text-red-600" title={event.error_message || ''}>
										{event.error_message || '-'}
									</td>
									<td class="p-3 text-right">
										<button class="rounded border px-2 py-1 text-sm" on:click={() => openEventDetail(event)}>
											상세
										</button>
									</td>
								</tr>
				...
				{#if historyTotalPages > 1}
					<div class="flex items-center justify-between border-t px-3 py-2">
						<div class="text-sm text-slate-500">{historyPage} / {historyTotalPages}</div>
