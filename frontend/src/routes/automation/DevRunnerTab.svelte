...
<div class="flex flex-col h-full overflow-hidden">
	...
	<div class="flex-1 min-h-0 flex flex-col sm:overflow-hidden">
		...
		{:else}
			...
			<!-- Phase 3: 러너 리스트 카드 (기존 탭 바 대체) -->
			<div class="px-2 sm:px-3 pt-1">
				<div class="bg-card border border-border rounded-lg shadow-sm overflow-hidden flex flex-col transition-all">
					<div class="flex items-center justify-between px-3 py-2 border-b border-border bg-muted/30">
						<div class="flex items-center gap-2">
							<span class="text-xs font-bold uppercase tracking-wider text-muted-foreground">Active Runners</span>
							{#if runningCount > 0}
						...
						<div class="flex items-center gap-1.5">
							...
							<button 
								...
								class="p-1.5 rounded hover:bg-muted text-muted-foreground transition-colors {activeTabId === '__logs__' ? 'text-primary bg-primary/10' : ''}"
								title="Logs"
							>
								<svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="4 17 10 11 4 5"/><line x1="12" y1="19" x2="20" y2="19"/></svg>
							</button>
							<button 
								onclick={() => activeTabId = '__merge__'}
								class="p-1.5 rounded hover:bg-muted text-muted-foreground transition-colors {activeTabId === '__merge__' ? 'text-primary bg-primary/10' : ''}"
								title="Merge Queue"
							>
								<svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="18" cy="18" r="3"/><circle cx="6" cy="6" r="3"/><path d="M6 21V9a9 9 0 0 0 9 9"/></svg>
							</button>
							
							<div class="w-px h-3 bg-border mx-1"></div>
							
							<button 
								onclick={() => runnerCardCollapsed = !runnerCardCollapsed}
								class="p-1 rounded hover:bg-muted text-muted-foreground transition-colors"
							>
								<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 transition-transform {runnerCardCollapsed ? 'rotate-180' : ''}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="18 15 12 9 6 15"/></svg>
							</button>
						</div>
					</div>

					{#if !runnerCardCollapsed}
						<div class="max-h-[140px] overflow-y-auto divide-y divide-border bg-white">
							{#if runnerTabs.length === 0}
								<div class="px-4 py-6 text-center text-xs text-muted-foreground italic">
									활성 러너가 없습니다. 상단 실행 버튼으로 시작하세요.
								</div>
							{:else}
								{#each runnerTabs as tab (tab.id)}
									<div 
										class="flex items-center gap-3 px-3 py-2 cursor-pointer transition-colors group {activeTabId === tab.id ? 'bg-primary/5' : 'hover:bg-muted/50'}"
									...
									>
										...
										<div class="flex-1 min-w-0">
											<div class="text-[11px] font-mono font-medium truncate {activeTabId === tab.id ? 'text-primary' : 'text-foreground'}">
												{tab.plan_file && tab.plan_file !== '__ALL_PLANS__' && tab.plan_file !== 'ALL' ? tab.plan_file.split(/[\\/]/).pop()?.replace(/^\d{4}-\d{2}-\d{2}_/, '') : '전체 실행'}
											</div>
											<div class="text-[9px] text-muted-foreground flex items-center gap-2">
												<span>{tab.id.slice(0, 8)}</span>
												{#if tab.running && tab.start_time}
													<span class="text-amber-600 font-mono">{elapsed}</span>
												{/if}
											</div>
										</div>

										<button
											class="p-1 rounded hover:bg-muted text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity"
