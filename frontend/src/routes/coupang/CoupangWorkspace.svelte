<script lang="ts">
  ...
  type CoupangTab = 'schedules' | 'history' | 'cancellation-history';

  function normalizeCoupangTab(value: string | null | undefined): CoupangTab {
    if (value === 'history' || value === 'cancellation-history') {
      return value;
    }
    return 'schedules';
  }

  let activeTab = $state<CoupangTab>('schedules');

  $effect(() => {
    if (!unified) return;
    activeTab = normalizeCoupangTab(sub ?? view);
  });
  ...
  const coupangTabs = $derived([
    { id: 'schedules', label: '일정', count: schedules.length || undefined, href: unified ? '/monitoring?type=coupang&view=schedules' : undefined },
    { id: 'history', label: '이력', href: unified ? '/monitoring?type=coupang&view=history' : undefined },
    { id: 'cancellation-history', label: '메가뷰티쇼 취소이력', href: unified ? '/monitoring?type=coupang&view=cancellation-history' : undefined }
  ]);
