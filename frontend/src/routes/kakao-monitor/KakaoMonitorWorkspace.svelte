<script lang="ts">
	...
	function normalizeKakaoTab(tab: string | null | undefined): Tab {
		if (tab === 'settings' || tab === 'history' || tab === 'windows') return tab;
		return 'dashboard';
	}

	let activeTab = $state<Tab>(normalizeKakaoTab(view ?? sub));
	let lastLoadedTab = $state<Tab | null>(null);
	const tabList = [
	...
	// ========== 초기 로드 ==========
	onMount(async () => {
		if (!unified) await loadTabData(activeTab);
	});
	...
	async function handleTabChange(tabId: string) {
		const tab = normalizeKakaoTab(tabId);
		activeTab = tab;
		await loadTabData(tab);
		if (!unified) return;
		goto(buildMonitoringHref({ type: 'kakao', view: tab }, $page.url), {
			replaceState: true,
			keepFocus: true,
			noScroll: true
		});
	...
	$effect(() => {
		if (!unified) return;
		const next = normalizeKakaoTab(view ?? sub);
		if (activeTab !== next) activeTab = next;
		if (lastLoadedTab !== next) void loadTabData(next);
	});
