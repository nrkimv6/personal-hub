	...
	async function loadResults() {
		...
		try {
			if (activeTab === 'disappeared') {
				...
				// DisappearedResult를 SearchResultListItem 형태로 변환
...
<div class="container mx-auto px-4 py-6">
	...
	<div class="flex justify-between items-center mb-6">
		<div>
			<h1 class="text-2xl font-bold text-foreground">검색결과 관리</h1>
		...
		{#if totalPages > 1}
			<div class="flex justify-center items-center gap-2 mt-6">
				<button
					...
					disabled={page <= 1}
					class="px-3 py-1.5 text-sm border border-border rounded-lg hover:bg-muted transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
				>
					◀ 이전
				</button>
