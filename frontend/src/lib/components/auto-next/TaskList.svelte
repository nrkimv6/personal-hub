<script lang="ts">
	...
	let { tasks, total, currentFilter, onFilterChange, onDelete }: Props = $props();
...
<div class="bg-white rounded-lg border">
	<div class="p-4 border-b flex items-center justify-between flex-wrap gap-2">
		<h2 class="font-semibold">작업 목록 <span class="text-sm text-gray-400 font-normal">({total})</span></h2>
			...
			<table class="w-full text-sm">
				...
				<tbody>
					{#each tasks as task (task.id)}
						...
						>
							<td class="px-4 py-2">
								<span class="inline-block px-2 py-0.5 rounded text-xs font-medium {statusBadge(task.status)}">
									{task.status}
								</span>
							</td>
							<td class="px-4 py-2 max-w-xs truncate" title={task.text}>{task.text}</td>
							<td class="px-4 py-2 hidden md:table-cell text-gray-500">{task.model_used || '-'}</td>
							<td class="px-4 py-2 hidden md:table-cell text-gray-500">{formatDuration(task.duration_seconds)}</td>
							<td class="px-4 py-2 hidden lg:table-cell text-gray-500">{task.input_tokens + task.output_tokens}</td>
							...
							<tr class="bg-gray-50">
								<td colspan="7" class="px-4 py-3">
									<div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
										...
										<div><span class="text-gray-400">소스:</span> {task.source_path}</div>
										<div><span class="text-gray-400">입력 토큰:</span> {task.input_tokens}</div>
										<div><span class="text-gray-400">출력 토큰:</span> {task.output_tokens}</div>
										<div><span class="text-gray-400">캐시(읽기):</span> {task.cache_read_tokens}</div>
										<div><span class="text-gray-400">캐시(생성):</span> {task.cache_creation_tokens}</div>
										{#if task.error_message}
											<div class="col-span-full text-red-600"><span class="text-gray-400">에러:</span> {task.error_message}</div>
										{/if}
										{#if task.started_at}
											<div><span class="text-gray-400">시작:</span> {formatDate(task.started_at)}</div>
										{/if}
										{#if task.finished_at}
											<div><span class="text-gray-400">완료:</span> {formatDate(task.finished_at)}</div>
										{/if}
									</div>
								</td>
							</tr>
