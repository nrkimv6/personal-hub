	...
	>
		<div class="bg-white rounded-lg shadow-xl w-full max-w-5xl mx-4 max-h-[90vh] overflow-hidden">
			...
			<div class="p-4 border-b flex items-center justify-between bg-gray-50">
				<div>
					<h3 class="text-lg font-semibold text-gray-900">슬롯 조회</h3>
					{#if business && item}
						<p class="text-sm text-gray-500">{business.name} - {item.name}</p>
					{/if}
				</div>
				<button
					type="button"
			...
			<div class="p-4 overflow-y-auto" style="max-height: calc(90vh - 120px);">
				...
				{#if result}
					...
					{#if item}
						<div class="bg-white rounded-lg border border-gray-200 p-4 mb-4">
							<h4 class="text-sm font-semibold text-gray-700 mb-3">모니터링 이력</h4>
							{#if loadingStats}
							...
							<div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
								...
								>
									<div class="flex items-center gap-3">
										<svg
											class="w-4 h-4 text-gray-400 transition-transform {expandedDates.has(
												dateSlots.date
											)
												? 'rotate-90'
												: ''}"
											...
											{#each dateSlots.slots as slot}
												...
												>
													<div class="flex items-center justify-between mb-1">
														<span class="font-medium text-gray-900 text-sm">{slot.time}</span>
