...
export function getLogLineVariant(line: ParsedLine, opts: VariantOptions = {}): LogLineVariantInfo {
	...
	if (line.tag === 'TOOL') {
		return {
			variant: 'tool',
			containerClass: `dr-log-line dr-log-line-tool flex items-start gap-2 py-0 leading-5 opacity-40 ${line.isStale ? 'opacity-20' : ''}`,
	...
	if (line.tag === 'RESULT') {
		return {
			variant: 'result',
			containerClass: `dr-log-line dr-log-line-result flex items-start gap-0 py-0 leading-5 ${opts.resultFailure ? 'opacity-100 bg-red-950/35 border-l-2 border-red-500 -mx-1 px-1 rounded' : 'opacity-60'} ${line.isStale ? 'opacity-20' : ''}`,
	...
	return {
		...
		bodyClass: `flex-1 min-w-0 break-all whitespace-pre-wrap ${line.tag === 'FAILURE' ? 'text-red-300 font-bold' : line.tag === 'HOLD' ? 'text-yellow-300 font-bold' : line.tag === 'ERROR' ? 'text-red-400' : line.tag === 'DONE' ? 'text-green-400' : 'text-gray-300'}`,
