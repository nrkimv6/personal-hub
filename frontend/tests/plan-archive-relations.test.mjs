...
const archiveRecordDetailSource = readFileSync(
  new URL('../src/routes/plans/archive-tab/ArchiveRecordDetailPanel.svelte', import.meta.url),
  'utf8',
);
...
test('ArchiveTab and PlanViewer render relation surfaces separately from recurrence', () => {
  ...
  assert.match(archiveTabSource, /relations=\{selectedRelations\}/);
  assert.match(archiveRecordDetailSource, /미해결 후속/);
