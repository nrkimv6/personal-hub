# Video Download Worker Watchdog Script
# Monitors the Video Download worker process and automatically restarts it if it crashes
# Usage: .\scripts\video-download-watchdog.ps1
...
$WorkerPidFile = Join-Path $PidDir "video_download_worker$PidSuffix.pid"
...
function Write-Log {
    ...
    # Also log to file
    $logFile = Join-Path $LogDir "video_download_watchdog.log"
    Add-Content -Path $logFile -Value $logMessage -Encoding UTF8
}

function Start-VideoDownloadWorker {
    $Timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $stdoutLogFile = Join-Path $LogDir "stdout_video_download_worker_$Timestamp.log"
    $stderrLogFile = Join-Path $LogDir "stderr_video_download_worker_$Timestamp.log"

    Write-Log "Starting Video Download worker process..."
    ...
    $workerProcess = Start-Process -FilePath $VenvPython `
        -ArgumentList "-m", "app.worker.video_download_worker" `
    ...
    Write-Log "Video Download worker started with PID: $($workerProcess.Id)"
    return $workerProcess.Id
}

function Test-VideoDownloadWorkerRunning {
...
# Main watchdog loop
Write-Log "=" * 50
Write-Log "Video Download Worker Watchdog Started"
...
# Initial check
if (-not (Test-VideoDownloadWorkerRunning)) {
    Write-Log "Video Download worker not running, starting..." "WARN"
    Start-VideoDownloadWorker
    $restartCount++
    $lastRestartTime = Get-Date
}
...
try {
    while ($true) {
        ...
        if (-not (Test-VideoDownloadWorkerRunning)) {
            Write-Log "Video Download worker process died!" "ERROR"

            # Check restart limit
            if ($restartCount -ge $MaxRestarts) {
                Write-Log "Maximum restart limit ($MaxRestarts) reached in ${RestartWindow}s window!" "ERROR"
                Write-Log "Please check the Video Download worker logs for the root cause." "ERROR"
                Write-Log "Watchdog stopping to prevent restart loop." "ERROR"
                break
            }

            # Restart
            Write-Log "Restarting Video Download worker (attempt $($restartCount + 1)/$MaxRestarts)..." "WARN"
            Start-VideoDownloadWorker
            $restartCount++
            $lastRestartTime = Get-Date
        }
    ...
    Write-Log "Video Download Watchdog error: $_" "ERROR"
}
finally {
    Write-Log "Video Download Watchdog stopped"
}
