        ...
        def _try_insert(recs, label=""):
            ...
            try:
                with pg_engine.begin() as dst:
                    dst.execute(
                        text(f"INSERT INTO {table_name} ({', '.join(recs[0].keys())}) "
                             f"VALUES ({', '.join(':' + k for k in recs[0].keys())})"),
                        recs,
                    )
                return len(recs), 0
            except Exception as e:
                    ...
                    if pg_fk_cols:
                        try:
                            ...
                            with pg_engine.begin() as dst2:
                                dst2.execute(
                                    text(f"INSERT INTO {table_name} ({', '.join(nullified[0].keys())}) "
                                         f"VALUES ({', '.join(':' + k for k in nullified[0].keys())})"),
                                    nullified,
                                )
                            print(f"  [FK null처리] {table_name} {label}: {len(pg_fk_cols)}개 컬럼 NULL 처리")
                            return len(nullified), 0
                        except Exception:
                ...
                for r in recs:
                    try:
                        with pg_engine.begin() as dst3:
                            dst3.execute(
                                text(f"INSERT INTO {table_name} ({', '.join(r.keys())}) "
                                     f"VALUES ({', '.join(':' + k for k in r.keys())})"),
                                [r],
                            )
                        row_ok += 1
                    except Exception:
    ...
    with pg_engine.begin() as conn:
        for table in SERIAL_TABLES:
            ...
            try:
                ...
                result = conn.execute(text(
                    "SELECT EXISTS (SELECT 1 FROM pg_sequences WHERE sequencename = :seq)"
                ), {"seq": seq_name}).scalar()
                if not result:
                    continue
...
def main():
    ...
    print(f"\n이관 대상: {len(tables)}개 테이블 (제외 {len(EXCLUDED_TABLES)}개)")
    ...
    if not args.dry_run:
        ...
        for pass_num in range(1, MAX_PASSES + 1):
            ...
            for table in pending_tables:
                migrated, failed = migrate_table(sqlite_engine, pg_engine, table, dry_run=False)
