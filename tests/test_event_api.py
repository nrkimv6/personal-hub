...
class TestEventUpdateAPI:
    ...
    def test_update_event_partial(self, client, sample_event):
        """부분 수정 (다른 필드 유지)"""
...
class TestEventDeleteAPI:
    ...
    def test_delete_event_not_found(self, client):
        """존재하지 않는 이벤트 삭제"""
        response = client.delete("/api/v1/events/99999")
        assert response.status_code == 404
...
class TestEventBookmarkAPI:
    ...
    def test_toggle_bookmark(self, client, sample_event):
        """북마크 토글"""
        # OFF -> ON
    ...
    def test_toggle_bookmark_not_found(self, client):
        """존재하지 않는 이벤트 북마크"""
        response = client.post("/api/v1/events/99999/bookmark")
        assert response.status_code == 404
...
class TestEventParticipateAPI:
    ...
    def test_toggle_participate(self, client, sample_event):
        """참여 완료 토글"""
        # OFF -> ON
...
class TestEventImportFromInstagramAPI:
    ...
    def test_import_from_instagram_with_title(self, client, instagram_post):
        """커스텀 제목으로 가져오기"""
        response = client.post("/api/v1/events/import-from-instagram", json={
            "instagram_post_id": instagram_post.id,
            "title": "내가 정한 제목",
