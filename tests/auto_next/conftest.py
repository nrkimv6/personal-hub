"""auto_next 테스트 공통 conftest - config 격리로 hang 방지"""
...
def auto_next_config_isolation(tmp_path):
    """모든 auto_next 테스트에서 config를 tmp 경로로 격리.
    ...
    with patch("app.modules.auto_next.services.plan_service.config", mock_config), \
         patch("app.modules.auto_next.services.db_service.config", mock_config):
        yield mock_config
