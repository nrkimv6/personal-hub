...
def _read_status_actions_py() -> str:
    """scripts/services/browser_worker_runtime/status_actions.py 소스 반환."""
    import os
    base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    path = os.path.join(base, "scripts", "services", "browser_worker_runtime", "status_actions.py")
    with open(path, encoding="utf-8") as f:
        return f.read()
...
class TestBrowserWorkersPubsubFlagCheck:
    ...
    def test_browser_workers_pubsub_flag_check(self):
        ...
        src = _read_status_actions_py()

        # 수정된 패턴이 있어야 함
        assert '"S" in c.get("flags"' in src or '"S" in c.get(' in src, (
            "browser_worker_runtime/status_actions.py에 '\"S\" in flags' 패턴이 없음. "
            "pubsub_count 판정이 수정되지 않음."
        )

        # 구버전 패턴이 없어야 함
        assert 'startswith("S")' not in src, (
            "browser_worker_runtime/status_actions.py에 flags.startswith('S') 패턴이 남아 있음. "
