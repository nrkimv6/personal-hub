...
class TestHttpE2EChain:
    """T5: TestClient 기반 HTTP E2E — isolated_redis_db15 + listener_process(db=15) 격리"""
    ...
    def cleanup_redis_after_test(self, isolated_redis_db15):
        ...
        try:
            ...
            for runner_id in active:
                pid_str = isolated_redis_db15.get(f"{RUNNER_KEY_PREFIX}:{runner_id}:pid")
                if pid_str:
                    try:
            ...
            # 4. Redis 키 전체 삭제 (isolated_redis_db15 = db=15)
            stale_keys = isolated_redis_db15.keys("plan-runner:*")
            if stale_keys:
    ...
    def test_http_start_and_stop_lifecycle(self, isolated_redis_db15, listener_process, isolated_plan_file):
        ...
        for _ in range(20):
            ...
            started_at = isolated_redis_db15.get(f"{RUNNER_KEY_PREFIX}:{runner_id}:started_at")
            in_recent = isolated_redis_db15.zscore(RECENT_RUNNERS_KEY, runner_id) is not None
        ...
        print(f"\n[START OK] runner_id={runner_id}, PID={isolated_redis_db15.get(f'{RUNNER_KEY_PREFIX}:{runner_id}:pid')}, accepted_at={accepted_at}")

        # 3. Stop runner
        resp = client.post(f"{BASE_URL}/stop/{runner_id}")
        if resp.status_code == 404:
            # dry_run이 이미 완료 → active_runners에서 직접 제거
            isolated_redis_db15.srem(ACTIVE_RUNNERS_KEY, runner_id)

        # 4. active_runners 비어짐 확인
        cleaned = False
        for _ in range(10):
            if not isolated_redis_db15.smembers(ACTIVE_RUNNERS_KEY):
                cleaned = True
                break
            time.sleep(1)
