...
def _wait_for_runner_status(isolated_redis_db15, runner_id: str, expected: str, timeout: int = 20) -> bool:
    """runner_id의 status 키가 expected 값이 될 때까지 폴링."""
    key = f"{RUNNER_KEY_PREFIX}:{runner_id}:status"
    for _ in range(timeout * 2):
        val = isolated_redis_db15.get(key)
        if val == expected:
            return True
        time.sleep(0.5)
    return False
...
def _wait_for_redis_key(isolated_redis_db15, key: str, timeout: int = 30) -> str | None:
...
class TestRunnerDryRun:
    ...
    async def test_dry_run_lifecycle(self, listener_process, isolated_redis_db15, e2e_redis_cleanup, e2e_worktree_cleanup):
        ...
        stopped = False
        for _ in range(20):
            status = isolated_redis_db15.get(f"{RUNNER_KEY_PREFIX}:{runner_id}:status")
    ...
    async def test_dry_run_redis_keys(self, listener_process, isolated_redis_db15, e2e_redis_cleanup, e2e_worktree_cleanup):
        ...
        async with httpx.AsyncClient(
            ...
            pid = isolated_redis_db15.get(f"{RUNNER_KEY_PREFIX}:{runner_id}:pid")
            plan_file = isolated_redis_db15.get(f"{RUNNER_KEY_PREFIX}:{runner_id}:plan_file")
            start_time = isolated_redis_db15.get(f"{RUNNER_KEY_PREFIX}:{runner_id}:start_time")
    ...
    async def test_dry_run_log_file_created(self, listener_process, isolated_redis_db15, e2e_redis_cleanup, e2e_worktree_cleanup):
        ...
        async with httpx.AsyncClient(
            ...
            log_path = _wait_for_redis_key(
                isolated_redis_db15, f"{RUNNER_KEY_PREFIX}:{runner_id}:stream_log_path", timeout=30
            )
    ...
    async def test_concurrent_dry_run(self, listener_process, isolated_redis_db15, e2e_redis_cleanup, e2e_worktree_cleanup, cleanup_test_branches):
        ...
        async with httpx.AsyncClient(
            ...
            for rid in (runner_id_1, runner_id_2):
                assert _wait_for_runner_status(isolated_redis_db15, rid, "running", timeout=60), (
                    f"runner {rid}가 60초 내 running 상태가 되지 않음"
                )

            for rid in (runner_id_1, runner_id_2):
                assert isolated_redis_db15.get(f"{RUNNER_KEY_PREFIX}:{rid}:pid") is not None, (
    ...
    async def test_accepted_started_ordering(self, listener_process, isolated_redis_db15, e2e_redis_cleanup, e2e_worktree_cleanup):
        ...
        async with httpx.AsyncClient(
            ...
            started_at_str = _wait_for_redis_key(
                isolated_redis_db15, f"{RUNNER_KEY_PREFIX}:{runner_id}:started_at", timeout=30
            )
            assert started_at_str is not None, f"started_at 미관찰 (runner_id={runner_id}, 30초)"

            accepted_at_str = isolated_redis_db15.get(f"{RUNNER_KEY_PREFIX}:{runner_id}:accepted_at")
