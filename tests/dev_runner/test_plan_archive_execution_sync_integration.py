"""Plan Archive 실행 결과 overwrite 방어 통합 테스트.

검증 범위:
1. 오래된 completed request가 최신 request 저장값을 덮어쓰지 않음
2. 최신 completed request만 PlanRecord에 반영됨
3. PlanArchiveExecutionService가 Codex provider를 profile 없이 큐잉함
"""
import json
import pytest
...
class TestHasNewerPlanArchiveResult:
    ...
    def test_no_newer_request_returns_false(self, db):
    ...
    def test_newer_completed_request_returns_true(self, db):
    ...
    def test_newer_pending_request_not_counted(self, db):
    ...
    def test_different_caller_id_not_counted(self, db):
...
class TestSaveOverwriteGuard:
    ...
    def test_older_request_does_not_overwrite_newer(self, db):
    ...
    def test_newest_request_applied_to_record(self, db):
    ...
    def test_plan_archive_analysis_saved_event_created(self, db):
        ...
        saved_detail = json.loads(events[-1].detail) if isinstance(events[-1].detail, str) else events[-1].detail
        assert saved_detail.get("request_id") == req.id

    def test_empty_result_dict_does_not_fail(self, db):
...
class TestPlanArchiveExecutionServiceCodex:
    ...
    def test_queue_analysis_codex_no_profile(self, db):
    ...
    def test_queue_analysis_unsupported_provider_raises(self, db):
    ...
    def test_queue_analysis_deduplicates_pending(self, db):
