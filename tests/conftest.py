...
def block_trigger_user_direct_write():
    ...
    단위 테스트에서 trigger='user'를 sync/async fakeredis에 직접 기록하는 코드를 탐지한다.
    autouse 대신 명시적으로 사용하여 격리 guard 자체를 검증하는 TC에서 활용한다.
    E2E 테스트는 allow_prod_redis 마커로 실서버 Redis를 사용하므로 이 fixture 불필요.
    """
    try:
        import fakeredis as _fakeredis
        import fakeredis.aioredis as _async_fakeredis
    except ImportError:
        yield
        return

    original_fake_set = _fakeredis.FakeRedis.set
    original_fake_async_set = _async_fakeredis.FakeRedis.set

    def _fail_on_user_trigger(name, value):
    ...
    def _guarded_fake_set(self, name, value, *args, **kwargs):
        _fail_on_user_trigger(name, value)
        return original_fake_set(self, name, value, *args, **kwargs)

    async def _guarded_fake_async_set(self, name, value, *args, **kwargs):
        _fail_on_user_trigger(name, value)
        return await original_fake_async_set(self, name, value, *args, **kwargs)

    with patch.object(_fakeredis.FakeRedis, "set", _guarded_fake_set), \
         patch.object(_async_fakeredis.FakeRedis, "set", _guarded_fake_async_set):
        yield
