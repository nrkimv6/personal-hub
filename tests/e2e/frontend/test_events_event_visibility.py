...
def test_events_filter_can_request_ongoing_or_upcoming(page: Page, frontend_url: str):
    ...
    seen_popup_statuses: list[str | None] = []
    ...
    def handle_events(route):
        ...
        seen_popup_statuses.append(request.url.split("popup_status=")[1].split("&")[0] if "popup_status=" in request.url else None)
        route.fulfill(
            status=200,
    ...
    assert all(value is None for value in seen_popup_statuses)


def test_popups_filter_can_request_ongoing_or_upcoming(page: Page, frontend_url: str):
    seen_popup_statuses: list[str | None] = []
    seen_event_statuses: list[str | None] = []

    def handle_popups(route):
        request = route.request
        seen_popup_statuses.append(request.url.split("popup_status=")[1].split("&")[0] if "popup_status=" in request.url else None)
        seen_event_statuses.append(request.url.split("event_status=")[1].split("&")[0] if "event_status=" in request.url else None)
        route.fulfill(
    ...
    page.route("**/api/v1/events/deadline-counts**", lambda route: route.fulfill(status=200, content_type="application/json", body="{}"))
    page.route("**/api/v1/popups?**", handle_popups)

    page.goto(f"{frontend_url}/events?tab=popup")
    page.wait_for_load_state("networkidle")
    ...
    assert "ongoing_or_upcoming" in seen_popup_statuses
    assert all(value is None for value in seen_event_statuses)
