...
class SiteMonitorFactory:
    ...
    @classmethod
    def create_service(cls, target: MonitorTarget, notification_service: Optional[NotificationService] = None, browser_service: Optional[BrowserService] = None) -> AbstractSiteMonitor:
        """
        모니터링 대상의 타입에 따라 적절한 서비스를 생성합니다.
        """
        service_class = cls._services.get(target.service_type)
        if not service_class:
            raise ValueError(f"지원하지 않는 서비스 타입입니다: {target.service_type}")
        
        return service_class(notification_service=notification_service, browser_service=browser_service) 