"""
파일 분류 전용 SQLite 데이터베이스 연결 모듈

기존 monitor-page DB와 분리하여 독립적으로 관리:
- 파일 위치: data/file_classifier.db
- 이유: 대용량 파일 목록, 독립 백업/삭제, 스키마 충돌 방지
"""
...
DB_PATH = DB_DIR / "file_classifier.db"
...
def init_db():
    ...
    try:
        for migration_file in migration_files:
            print(f"[파일 분류 DB] 마이그레이션 실행: {migration_file.name}")
        ...
        print(f"[파일 분류 DB] 초기화 완료: {DB_PATH}")
    except Exception as e:
        db.rollback()
        print(f"[오류] 파일 분류 DB 초기화 실패: {e}")
        raise
    finally:
        db.close()
