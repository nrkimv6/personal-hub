...
class TestGetTasks:
    async def test_get_tasks_returns_200(self, client, patch_db):
        response = await client.get("/api/auto-next/tasks")
    ...
    async def test_get_tasks_with_status_filter(self, client, patch_db):
        response = await client.get("/api/auto-next/tasks?status=pending")
        data = response.json()
        assert data["total"] == 1
        assert data["tasks"][0]["status"] == "pending"

    async def test_get_tasks_with_limit(self, client, patch_db):
        response = await client.get("/api/auto-next/tasks?limit=1")
        data = response.json()
...
class TestGetTask:
    async def test_get_task_found(self, client, patch_db):
        response = await client.get("/api/auto-next/tasks/test-1")
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_get_task_not_found(self, client, patch_db):
        response = await client.get("/api/auto-next/tasks/nonexistent")
        assert response.status_code == 404


class TestDeleteTask:
    async def test_delete_task_success(self, client, patch_db):
        response = await client.delete("/api/auto-next/tasks/test-2")
        assert response.status_code == 200
        response = await client.get("/api/auto-next/tasks/test-2")
        assert response.status_code == 404

    async def test_delete_task_not_found(self, client, patch_db):
        response = await client.delete("/api/auto-next/tasks/nonexistent")
        assert response.status_code == 404


class TestGetStats:
    async def test_get_stats_returns_200(self, client, patch_db):
        response = await client.get("/api/auto-next/stats")
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 3
    ...
    async def test_get_history(self, client, patch_db):
        response = await client.get("/api/auto-next/history")
        assert response.status_code == 200
        assert isinstance(response.json(), list)

    async def test_get_duplicates(self, client, patch_db):
        response = await client.get("/api/auto-next/duplicates")
        assert response.status_code == 200
        assert isinstance(response.json(), list)
