...
# 이벤트 JSON 스키마
EVENT_JSON_SCHEMA = """{
    "is_event": true|false (이벤트/행사/프로모션인지 여부, 필수),
    "title": "이벤트 제목 (is_event=true인 경우 필수, 아니면 페이지 제목)",
    ...
    "not_event_reason": "이벤트가 아닌 경우 이유 (is_event=false인 경우 필수)"
}"""
...
## 분석 요청
먼저 이 페이지가 이벤트/행사/프로모션 페이지인지 판단해주세요.
이벤트가 맞다면 다음 항목들을 추출해주세요:
...
중요:
- is_event 필드는 반드시 true/false로 입력해주세요
- 이벤트가 아니면 is_event=false로 설정하고, not_event_reason에 이유를 적어주세요
...
def parse_event_from_llm_response(response: str) -> Dict[str, Any] | None:
    ...
    Returns:
        ...
        - is_event=True인 경우: 이벤트 정보 딕셔너리
        - is_event=False인 경우: {"is_event": False, "not_event_reason": "...", ...} 형태
    """
    import json
    import re
    ...
    try:
        ...
        # is_event 필드 처리 (기본값 True - 하위 호환성)
        is_event = data.get("is_event", True)
        if isinstance(is_event, str):
            is_event = is_event.lower() == "true"
        data["is_event"] = is_event

        # 비이벤트인 경우 기본값만 설정하고 반환
        if not is_event:
