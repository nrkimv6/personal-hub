...
네이버 예약 시스템은 `/request` URL로 직접 접속할 수 없습니다.
반드시 **상품 상세 페이지 → 시간 선택 → "다음" 버튼 클릭** 순서로 진행해야 합니다.

**이유:**
- 시간 선택 시 `slotId`, `scheduleId` 등이 세션/쿠키에 저장됨
- GraphQL API 호출로 회차 정보 조회 필요
- React 상태 또는 쿠키에 예약 정보가 저장되어야 `/request` 페이지가 작동함
...
2. 🔒 탭 독점 모드 진입 (모니터링 중단)
   - 예약 진행 중 다른 페이지 이동 차단
   - Fetch API 요청 차단
   - 탭을 예약 프로세스만 사용

3. [STEP 1] 상품 상세 페이지 접속
   - URL 형식: /booking/{category}/bizes/{businessId}/items/{itemId}?startDateTime=...
   - startDateTime 파라미터로 날짜 자동 선택

4. [STEP 2] 날짜 확인
   - URL 파라미터로 이미 선택되어 있는지 확인
   - 선택자: button.calendar_date.selected

5. [STEP 3] 시간 선택 ⭐ 핵심!
   - 24시간 형식을 오전/오후 형식으로 변환 (21:00 → 오후 9:00)
   - button.btn_time 중에서 일치하는 시간 찾기
   - 클릭하여 시간 선택
   - 시간을 찾을 수 없으면 사용 가능한 시간 목록 출력 후 종료

6. [STEP 4] "다음" 버튼 클릭 ⭐ 핵심!
   - 선택자 시도 순서:
     1. button.NextButton__btn_next__kfLFW (발견된 실제 선택자)
     2. button:has-text("다음")
     3. button.btn_next
     4. button[class*="btn_next"]
   - 클릭 후 /request 페이지로 이동 대기
   - GraphQL API 호출 발생:
     * schedule: 일정 조회 (slotId, scheduleId 획득)
     * bookingRequestSupply: 예매 양식 정보
     * account: 사용자 정보 확인

7. [STEP 5] 인원 확인
   - button.select_btn 확인 (필요시)

8. [STEP 6] 예매 버튼 활성화 대기
   - button.btn_request:not(.disabled) 대기
   - 타임아웃: 10초

9. [STEP 7] 예매 버튼 클릭
   - DRY_RUN_MODE=True: 시뮬레이션만 (실제 클릭 안함)
   - DRY_RUN_MODE=False: button.btn_request 클릭 ⚠️
...
**주요 개선 사항 (2025-11-29):**
- ✅ `/request` 직접 접속 → 상품 상세 페이지에서 시작
- ✅ 24시간 형식을 오전/오후 형식으로 자동 변환
- ✅ "다음" 버튼 선택자 여러 가지 시도
- ✅ 시간 찾기 실패 시 사용 가능한 시간 목록 출력
- ✅ 상세한 에러 추적 (traceback)
...
### ⚠️ "/request URL로 직접 접속할 수 없습니다" 오류 (해결됨)

**문제:**
- 이전 버전에서 `/request` URL로 직접 접속 시도
- "예매 버튼이 활성화되지 않음" 또는 "페이지가 비어 있음" 오류 발생

**원인:**
- 네이버 예약 시스템은 `/request` 페이지 접속 전에 필수 정보가 세션/쿠키에 저장되어야 함:
  - `slotId`: 선택한 시간의 회차 ID
  - `scheduleId`, `detailScheduleId`: 일정 ID
  - `sessionId`: GraphQL 세션 ID
- 이 정보들은 상품 상세 페이지에서 시간 선택 시 GraphQL API 호출로 생성됨

**해결 방법 (2025-11-29 적용):**
```python
# ❌ 이전 방식 (작동 안함)
booking_url = f"https://booking.naver.com/booking/12/bizes/1515818/items/7127905/request?startDateTime=..."
await page.goto(booking_url)

# ✅ 새 방식 (올바른 플로우)
# 1. 상품 상세 페이지 접속
detail_url = f"https://booking.naver.com/booking/12/bizes/1515818/items/7127905?startDateTime=..."
await page.goto(detail_url)

# 2. 시간 선택 (오전/오후 형식으로 매칭)
await page.click('button.btn_time:has-text("오후 9:00")')

# 3. "다음" 버튼 클릭 → /request로 이동
await page.click('button.NextButton__btn_next__kfLFW')
await page.wait_for_url('**/request**')
...
**참고:**
- 분석 과정은 `old/2025-11-29_이슈_해결방안.md` 참조
- GraphQL API 호출 상세:
  - `schedule`: 일정 조회 (slotId 획득)
  - `bookingRequestSupply`: 예매 양식 정보
  - `account`: 사용자 정보 확인
...
**문제:**
- 슬롯 시간이 24시간 형식 (21:00)
- 화면 버튼은 12시간 형식 (오후 9:00)
- 매칭 실패로 시간 선택 불가

**해결:**
자동 변환 로직 적용됨 (auto_booking_monitor.py:202-211)
```python
# 24시간 → 12시간 변환
...
**문제:**
- CSS 클래스명이 동적으로 생성됨 (예: `NextButton__btn_next__kfLFW`)
- 단일 선택자로는 매칭 실패 가능

**해결:**
여러 선택자를 순차적으로 시도 (auto_booking_monitor.py:284-299)
```python
next_button_selectors = [
    'button.NextButton__btn_next__kfLFW',  # 현재 발견된 선택자
    'button:has-text("다음")',              # 텍스트 기반
    'button.btn_next',                      # 일반적인 클래스명
    'button[class*="btn_next"]'             # 부분 매칭
]
```
